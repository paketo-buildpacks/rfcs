...
### Dependency Version Resolution & Fallback

The `pnpm` CLI buildpack's embedded dependency metadata (used as a fallback when the primary, buildpack.toml-based resolver has no match — e.g. for a pnpm patch release too new to already be packaged) won't always have the exact version a project requests. This matters specifically for the `packageManager` field case: `package.json` frequently pins an exact version (e.g. via `corepack enable` or `npm create`), and pnpm v9+ refuses to run at all if the actually-running pnpm version doesn't match that field exactly (`ERR_PNPM_BAD_PM_VERSION`).

Rather than failing the build outright whenever the embedded metadata is a patch release or two behind, the fallback resolver substitutes the closest available version within the same major line and logs a clear warning naming both the requested and substituted versions. `BP_PNPM_STRICT_VERSION_MATCH=true` (on the `pnpm` buildpack) disables this leniency for fully-specified `major.minor.patch` requests and fails the build instead — a bare major-only request (e.g. `BP_PNPM_VERSION=9`) is unaffected either way, since that's already an intentional "give me the best available 9.x" ask rather than an exact pin.

When a substitution happens, `pnpm` sets `PNPM_VERSION_MISMATCH=true` as a `SharedEnv` variable on its layer, which `pnpm-install` (running later in the same order group) reads to decide whether it's safe to leave pnpm's own version-match enforcement in place, or whether it needs to relax it — see "Installation & Caching Logic" below. In the common case where the exact requested version is available, this is `false` and pnpm's own reproducibility guarantee is left untouched.
...
- **Environment Variables**:
  ...
  - `BP_PNPM_STRICT_VERSION_MATCH` (on the `pnpm` buildpack): Setting this to `true` disables silently substituting a same-major version when the exact requested version isn't in the embedded dependency metadata, failing the build instead (see "Dependency Version Resolution & Fallback" above). Defaults to `false`.
...
When `PNPM_VERSION_MISMATCH=true` (see "Dependency Version Resolution & Fallback" above), `pnpm-install` additionally relaxes pnpm's own `packageManager`-mismatch enforcement, since running `pnpm install` normally in that situation would fail outright with `ERR_PNPM_BAD_PM_VERSION` before ever reaching the dependency install itself. The specific setting to relax has changed across pnpm versions — v9 and v10 use `--config.package-manager-strict=false`; v11 removed that setting entirely in favor of `--config.pmOnFail=warn` — so `pnpm-install` checks the resolved pnpm major version (the same check used for the build-scripts flag above, reusing a single `pnpm --version` call when both checks are active) and applies whichever flag the running version actually supports. This relaxation is scoped specifically to a real, signaled mismatch — it is not applied on every build, since pnpm's version-match enforcement is a legitimate reproducibility guarantee for the common case where the delivered version matches exactly.
...
- **Cross-buildpack communication via `SharedEnv`**: `PNPM_VERSION_MISMATCH` is used to let `pnpm-install` know when the `pnpm` buildpack had to substitute a version, so it can selectively relax pnpm's own reproducibility check only in that case. This is a departure from the otherwise strict separation between the two buildpacks (each only talks to the other via the standard CNB build plan `Provides`/`Requires` contract), since the mismatch outcome isn't known until partway through `pnpm`'s own build phase, after detection has already run. Worth confirming this is an acceptable pattern, or whether the maintainers would prefer this surfaced through the build plan instead.
