...
This RFC further proposes to create a new builder repository that will create two builders, both using the build image as a base layer and the `base` run image as the default `run` image. One builder will not contain any buildpacks and the other will contain Paketo buildpacks.
