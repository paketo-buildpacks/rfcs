...
image (AKA "scratch" image) with few-to-no packages/libraries. Scratch images
...
own scratch stack. We have evidence of users doing both of these things.
...
Creating a scratch stack will decouple these use cases, allowing us to tailor
...
`paketo-buildpacks/jammy-scratch-stack` and a new pair of images on Dockerhub
at `paketobuildpacks/build-jammy-scratch` and
`paketobuildpacks/run-jammy-scratch`.

We will adopt the tag system used for Jammy - specifically semver (i.e.
'X.Y.Z') and 'latest' tags.

We will not create a builder repository or image, though we reserve the option
for the Builder team to do so in the future if that is requested and approved.

### Stack ID

We propose creating a new stack ID: `io.buildpacks.stacks.jammy.scratch`.
This may require approval from the upstream Cloud-Native Buildpacks project.

### Image Contents

The scratch stack run image will contain the `tzdata` and `ca-certificates`
packages, the standard CNB/Paketo metadata (including users and /etc/passwd
entries), and nothing else. These two packages are often provided in scratch
...
In practice, the scratch stack will look like a stripped-down version of the
...
Although there are very few CVEs for the `tzdata` and `ca-certificates`
packages, and hence the run image will rarely be updated, we want to ensure the
...
1. The scratch stack can use the Golang buildpack for its integration tests.
...
1. We could not create a scratch stack at all, and continue adding more
...
1. Create a new Github repository at: `paketo-buildpacks/jammy-scratch-stack`
...
commercial/internal environments. Their feedback is that a scratch image with
...
describe the use of "scratch" images for statically-linked golang applications,
with or without the `tzdata` and `ca-certificates` packages.

## Unresolved Questions and Bikeshedding

* Is the name "scratch" misleading, given we propose adding one or two
  packages? Should we create a true scratch image in addition to a this image?
  If so, what should we call the currently-proposed image with one or two
  packages? "Minimal"? "Tiniest"? "Atomic"? "Miniscule"?
